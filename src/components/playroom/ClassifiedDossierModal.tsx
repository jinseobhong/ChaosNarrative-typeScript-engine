  ...
  return (
    ...
    >
      <div className="space-y-4 text-xs">
        {/* Banner with Character Portrait */}
        <div className="p-3.5 bg-red-950/40 border border-red-500/40 rounded-xl flex items-center justify-between text-red-200 font-mono">
          <div className="flex items-center gap-3">
            {character.avatarUrl ? (
              <img
                src={character.avatarUrl}
                alt={character.name}
                className="w-12 h-14 rounded-lg object-cover border border-red-400 shadow-md shrink-0"
              />
            ) : (
              <div
                className={`w-12 h-12 rounded-lg ${character.avatarColor} flex items-center justify-center font-serif text-xl font-bold text-white shadow shrink-0`}
              >
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-sm text-white">{character.name} ({character.title})</span>
              </div>
              <div className="text-[11px] text-red-300/80 mt-0.5">
                {character.code} • {character.archetype} ({character.archetypeDetail})
              </div>
            </div>
          </div>
          <span className="text-[10px] text-red-400 bg-red-950/80 px-2 py-1 rounded border border-red-500/30">
